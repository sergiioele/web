...
function insertaVG($idA)
{
	...
	$qry="insert into ventas(idAuto,idUsuario,fechaventa,total,estadoventa) values(".$idA.",".$_SESSION['idUs'].",'".$d."',".$tot.",'Pendiente')";
	mysql_query($qry) or die("Error al insertar Auto".mysql_error());
		$mm=1;
	$qry="update autos set disponibilidad=".$mm." where idAuto=".$idA;
	mysql_query($qry) or die("Error al insertar Auto".mysql_error());
...
function insertaR($idA,$idC,$fs,$fe,$esta)
{
		...
		$datos = mysql_fetch_object($rs) or die("no se pudo conseguir informacion del auto");
		echo $datos->marca;
...
function insertaU($nom,$con,$tipo)
{
	...
	{
		$qry="insert into usuarios(nombreus,contrasena,tipousuario) values('".$nom."','".$con."','".$tipo."')";
...
function modificaU($idUsu,$nom,$con,$tipo)
{
	...
	$qry="update usuarios set nombreus='".$nom."', contrasena='".$con."', tipousuario='".$tipo."' where idUsuario=".$idUsu;
